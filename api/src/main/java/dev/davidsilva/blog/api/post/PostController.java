...
public class PostController {
    ...
    */
    ...
    @GetMapping
    public ResponseEntity<Page<Post>> getPosts(
            // TODO take field names and values from string like in the commented out function above
            // @RequestParam(value = "search", required = false) String search
    ) {
        SearchCriteria searchCriteria = SearchCriteria.Builder.create();
        searchCriteria.add(new SearchCriterion("id", SearchCriterion.Operator.GT, 0));
        Pageable pageable = PageRequest.of(0, 10);
        Page<Post> paginatedPosts = postService.searchPosts(searchCriteria, pageable);
        return new ResponseEntity<>(paginatedPosts, HttpStatus.OK);
    }
